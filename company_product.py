...
if not os.path.exists(company_path):
    ...
    shortname_allname = {}
    for file in os.listdir(text_path):
        company_year = file.split("__")[-3:-1]
        shortname_year = "__".join(company_year)
        all_name = file
        shortname_allname[shortname_year] = all_name
    json.dump(
        shortname_allname,
        open(f"{company_path}/shortname_allname.json", "w", encoding="utf-8"),
        indent=2,
        ensure_ascii=False,
    )
